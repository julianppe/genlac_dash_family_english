...
dash.register_page(__name__,
                   path='/desired-fertility-rate',  # represents the url text
                   name='Desired fertility rate',  # name of page, commonly used as name of link
                   title='Desired fertility rate'  # epresents the title of browser's tab
)
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Women/men ratio', id='page21-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
